...
@section('title', 'TMBL | Quote: '.$details['fields']['client']->first_name.' '.$details['fields']['client']->last_name .' [ID '.$details['fields']['record']->id.']')

@section('preheader')
    Quote Sent: {{$details['fields']['client']->first_name}} {{$details['fields']['client']->last_name}} [ID {{$details['fields']['record']->id}}]
@endsection
...
<table border="0" align="center" cellpadding="10" cellspacing="0" width="100%">
    <tr>
        <td>
            <p>Dear {{$details['fields']['adviser']->first_name}}</p>

            <p>The quote below has been sent to your client (without the provider name) at {{$details['fields']['record']->created_at}}:</p>
            ...
            <table border="0" align="center" cellpadding="0" cellspacing="0" width="1280" id="productColumns">
                ...
                @foreach(json_decode($details['fields']['record']->options) as $option)
                    <td align="center" valign="top" class="productColumnContainer">
                        ...
                        @include('templated.'.$details['fields']['adviser']->account->viewset.'.quote._partials.option_adviser',[
                            'option' => $option
                            , 'loop' => $loop
