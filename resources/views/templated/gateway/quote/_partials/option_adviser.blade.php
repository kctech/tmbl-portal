        ...
        <tr>
            ...
            <td class="oddRow">
                @if($fee_type == "Fixed Fee")
                    &pound;{{number_format($fee,2)}} on {{$fee_timing}}
                @elseif($fee_type == "Percentage")
                    {{number_format($fee,2)}}% on {{$fee_timing}}
                @else 
                    No Fee
                @endif
                ...
                @if($fee_2_type == "Fixed Fee")
                    &pound;{{number_format($fee_2,2)}} on {{$fee_2_timing}}<br />
                @elseif($fee_2_type == "Percentage")
                    {{number_format($fee_2,2)}}% on {{$fee_2_timing}}<br />
                @endif

                Total: &pound;{{number_format($option->tmbl_fee,2)}}
            </td>
        </tr>
        <tr>
            <td>
