...
@section('title') BTL Confirmation Requests @endsection

@section('breadcrumbs')
    {{ Breadcrumbs::render('btl-consent') }}
    <a href="{{ route('btl-consent.create') }}" class="btn btn-lg btn-primary ml-3 mb-3"><i class="fa fa-plus-circle"></i> Add New</a>
@endsection
...
<div class="card mb-4">
    <div class="card-body">
        <h3>Filters</h3>
        <form class="d-flex align-items-center justify-content-start" method="post" action="{{ route('btl-consent.search') }}">
            @csrf
            ...
            <div class="ml-auto">
                <a href="{{ route('btl-consent.index') }}" class="btn btn-outline-dark btn-lg">Clear</a>
        ...
        <table class="table table-bordered table-striped table-hover text-sm">
            ...
            <tbody>
                ...
                @foreach($requests as $request)
                    ...
                    <tr class="">
                        <td><a href="{{ route('btl-consent.edit',$request->id)}}" class="btn btn-block btn-sm btn-dark" data-toggle="tooltip" data-placement="right" title="Edit request"><i class="far fa-edit"></i> {{ $request->id }}</a></td>
                        <td>
                        ...
                        <td>
                            @if($request->consent == 'N')
                                <a href="{{signedRoute('btl-consent.respond', ['code'=>$request->client->uid, 'id'=>$request->id])}}" class="btn btn-block btn-sm btn-danger" target="_blank" data-toggle="tooltip" data-placement="right" title="Open client response page">No (copy link <i class="far fa-external-link"></i>)</a>
                            @else
                                <button type="button" data-href="{{route('btl-consent.resend-adviser', $request->id)}}" class="btn btn-block btn-sm btn-success action-btn" data-toggle="tooltip" data-placement="right" title="Resend email to yourself">Yes <i class="far fa-sync"></i> Adviser <i class="far fa-envelope"></i></button>
                            @endif
                        </td>
                        ...
                        <td>
                            @if($request->consent == 'N')
                                <button type="button" data-href="{{route('btl-consent.resend-client', $request->id)}}" class="btn btn-block btn-sm btn-warning action-btn" data-toggle="tooltip" data-placement="right" title="Resend email to client"><i class="far fa-sync fa-spin"></i> {{ $request->client->email }}</button>
                            @else
