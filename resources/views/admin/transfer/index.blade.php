...
@section('title') Client Transfer (from Openwork) Requests @endsection

@section('breadcrumbs')
    {{ Breadcrumbs::render('transfer-request') }}
    <a href="{{ route('transfer-request.create') }}" class="btn btn-lg btn-primary ml-3 mb-3"><i class="fa fa-plus-circle"></i> Add New</a>
@endsection
...
<div class="card mb-4">
    <div class="card-body">
        <h3>Filters</h3>
        <form class="d-flex align-items-center justify-content-start" method="post" action="{{ route('transfer-request.search') }}">
            @csrf
            ...
            <div class="ml-auto">
                <a href="{{ route('transfer-request.index') }}" class="btn btn-outline-dark btn-lg">Clear</a>
        ...
        <table class="table table-bordered table-striped table-hover text-sm">
            ...
            <tbody>
                ...
                @foreach($requests as $request)
                    ...
                    <tr class="">
                        <td><a href="{{ route('transfer-request.edit',$request->id)}}" class="btn btn-block btn-sm btn-dark" data-toggle="tooltip" data-placement="right" title="Edit request"><i class="far fa-edit"></i> {{ $request->id }}</a></td>
                        <td>
                            @if($request->consent == 'N')
                                <a href="{{signedRoute('transfer-request.respond', ['code'=>$request->client->uid, 'id'=>$request->id])}}" class="btn btn-block btn-sm btn-danger" target="_blank" data-toggle="tooltip" data-placement="right" title="Open client response page">No (copy link <i class="far fa-external-link"></i>)</a>
                            @else
                                <button type="button" data-href="{{route('transfer-request.resend-adviser', $request->id)}}" class="btn btn-block btn-sm btn-success action-btn" data-toggle="tooltip" data-placement="right" title="Resend email to yourself">Yes <i class="far fa-sync"></i> Adviser <i class="far fa-envelope"></i></button>
                            @endif
                        </td>
                        ...
                        <td>
                            @if($request->consent == 'N')
                                <button type="button" data-href="{{route('transfer-request.resend-client', $request->id)}}" class="btn btn-block btn-sm btn-warning action-btn" data-toggle="tooltip" data-placement="right" title="Resend email to client"><i class="far fa-sync fa-spin"></i> {{ $request->client->email }}</button>
                            @else
