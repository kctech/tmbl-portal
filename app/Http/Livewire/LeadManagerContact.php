...
class LeadManagerContact extends Component
{
    ...
    public function hide_contact()
    {
        $this->show_contact = false;
        $this->show_meeting = false;
        $this->selected_adviser = null;
        $this->selected_date = null;
        $this->selected_time = null;
    }

    public function show_meeting_contact()
    {
    ...
    public function allocate(){
        ...
        if($lead){
            ...
            $lead->save();
            $this->emit('updated', ['message' => "Lead allocated [" . $lead->id . "]"]);
        }else{
            $this->emit('error', ['message' => "Cant find lead [" . $lead->id . "]"]);
        }
        session()->flash('alert-success','Lead ID '.$this->lead_id.' has been allocated to '.$adviser->first_name.' '.$adviser->last_name);
        return $this->redirectRoute($this->redirect);
    }
                ...
                if($mab_lead_response->status){
                    ...
                    if(!empty($this->selected_date) && !empty($this->selected_time)){
                        ...
                        try {
                            ...
                            //no teams setup
                            $this->skipRender();
                            session()->flash('alert-success','Lead ID '.$this->lead_id.' transferred to '.$this->selected_adviser." but a Teams meeting couldn't be booked");
                            return $this->redirectRoute($this->redirect);
                        }
                    }else{
                        $this->skipRender();
